...
const EditAccountInfoScreen = ({ navigation }) => {
  ...
  const HandleSave = async () => {
    ...
    const validation = [
      { field: 'Business Name', value: businessName, min: 3, max: 100 },
      { field: 'About', value: aboutUs, min: 20, max: 500 },
      ...
      { field: 'Password', value: password, min: 6, max: 25, regExp: REGULAR_EXPRESSION.PASSWORD, isRequired: false },
      { field: 'Primary Phone', value: primaryPhone, min: 11, max: 11, regExp: REGULAR_EXPRESSION.PHONE, isRequired: true },
      { field: 'Secondary Phone', value: secondaryPhone, min: 11, max: 11, regExp: REGULAR_EXPRESSION.PHONE, isRequired: false },
    ]

    for (let i = 0; i < validation.length; i++) {
      let response = InputStringValidation(validation[i])
      if (response != null) {
        setAlert({ message: response, positiveCallback: () => setAlert(null) })
        return
      }
    }
            ...
            <CustomButton
              ...
              onPress={() => {
                if (
                  ...
                  `${businessName}`.trim() === initialData.businessName &&
                  `${aboutUs}`.trim() === initialData.aboutUs &&
