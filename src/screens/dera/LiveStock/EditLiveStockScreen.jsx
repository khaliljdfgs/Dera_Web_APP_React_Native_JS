...
import { LIVESTOCK_STATUS, NOTIFICATION_TYPES } from '../../../config/values'

import IMAGE_PLACEHOLDER from '../../../../assets/images/image-placeholder.png'

const EditLiveStockScreen = ({ navigation, route }) => {
  const screenWidth = Dimensions.get('window').width

  const [isLoading, setIsLoading] = useState(null)
  const [alert, setAlert] = useState(null)

  const { liveStock, isFromHomeScreen } = route.params

  // To Store User Input Values
  const [title, setTitle] = useState(liveStock.liveStockTitle || '')
  const [price, setPrice] = useState(`${liveStock.liveStockPrice}` || '')
  const [description, setDescription] = useState(liveStock.liveStockDescription || '')
  const [features, setFeatures] = useState(liveStock.liveStockFeatures || {})
  const [images, setImages] = useState(liveStock.liveStockPhotos || ['', '', '', '', ''])
  const [uploadedImages, setUploadedImages] = useState(liveStock.liveStockPhotos || ['', '', '', '', ''])
  const [soldOutTo, setSoldOutTo] = useState(liveStock.soldOutTo || { id: '', fullname: '' })
  const [buyerInfo, setBuyerInfo] = useState({})

  const [categoryDropDownOpen, setCategoryDropDownOpen] = useState(false)
  const [categoryDropDownValue, setCategoryDropDownValue] = useState(liveStock.liveStockCategory || '')
  const [categoryDropDownItems, setCategoryDropDownItems] = useState(LiveStockCategoryValues)

  const Prepare = async () => {
    setIsLoading('Loading...')

    await AsyncStorage.setItem('liveStockFeatures', JSON.stringify(features))
    await AsyncStorage.setItem('liveStockImages', JSON.stringify(images))
    await AsyncStorage.setItem('liveStockSoldOutTo', JSON.stringify(soldOutTo))

    const consumerUsersData = JSON.parse(await AsyncStorage.getItem('consumerUsers'))
    const soldOutToUser = JSON.parse(await AsyncStorage.getItem('consumerUsers')).filter(x => x.uid === liveStock.soldOutTo)[0]

    setSoldOutTo({
      id: liveStock.soldOutTo,
      fullname: consumerUsersData?.filter(x => x.id === liveStock.soldOutTo)[0]?.fullname
    })

    setBuyerInfo(soldOutToUser)
  ...
  const FreeUpMemory = async () => {
    ...
    await AsyncStorage.removeItem('liveStockSoldOutTo')
  }
  ...
  const Refresh = async () => {
    ...
    const _soldOutTo = JSON.parse(await AsyncStorage.getItem('liveStockSoldOutTo')) || soldOutTo
    if (_soldOutTo.id) setSoldOutTo(_soldOutTo)
  ...
  const SaveData = async () => {
    ...
    if (soldOutTo.id) {
      data.soldOutTo = soldOutTo.id
      data.liveStockStatus = LIVESTOCK_STATUS.SOLD_OUT

      const notificationPayload = {
        type: NOTIFICATION_TYPES.LIVESTOCK_SOLD_OUT,
        liveStockId: liveStock.id,
        receiver: soldOutTo.id,
      ...
      data.soldOutTo = ''
      data.liveStockStatus = LIVESTOCK_STATUS.AVAILABLE
    }

    setIsLoading('Updating Livestock...')
    await updateDoc(doc(Firestore, "livestocks", liveStock.id), data)
    setIsLoading(null)
    ...
    try {
      ...
      setAlert({
        ...
        positiveCallback: async () => {
          if (isFromHomeScreen) {
            await AsyncStorage.setItem('EditLiveStockGoToHomeScreen', 'true')
          }
          navigation.goBack()
        }
      })
  ...
  if (isLoading) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.COLOR_06 }}>
        <CustomAppBar title='Edit Livestock ' onGoBack={() => navigation.goBack()} />
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.COLOR_06 }}>
      ...
      <KeyboardAwareScrollView enableOnAndroid={true} enableAutomaticScroll={true}>
        <View style={{ flex: 1, padding: 24, gap: 16 }}>
          ...
          {
            liveStock.requests && liveStock.liveStockStatus === LIVESTOCK_STATUS.AVAILABLE &&
            <CustomTextInput
              value={soldOutTo.fullname}
              label='Sold Out To'
              placeholder="Select Consumer"
              selectMode={true}
              selectModeOnPress={() => {
                const payload = { requests: liveStock.requests || [] }
                navigation.navigate(ROUTES.DERA.SELECT_CONSUMER, payload)
              }}
              rightIcon={<MaterialIcons name='clear' color={COLORS.COLOR_01} size={20} />}
              onRightIconPress={() => setSoldOutTo({ id: '', fullname: '' })}
            />
          }

          {
            ...
            <CustomTable
              ...
              data={[
                { left: 'Fullname', right: buyerInfo.fullname },
                { left: 'Email', right: buyerInfo.email },
                {
                  left: 'Phone',
                  right: (buyerInfo.phone && `${buyerInfo.phone?.primary}${buyerInfo.phone?.secondary ? `\n${buyerInfo.phone?.secondary}` : ''}`) || 'N/A',
                },
                { left: 'Distance', right: buyerInfo.distanceString },
                { left: 'Address', right: buyerInfo.address },
              ]}
...
export default EditLiveStockScreen
