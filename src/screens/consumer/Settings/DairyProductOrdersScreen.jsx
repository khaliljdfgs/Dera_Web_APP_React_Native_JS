...
import { query, collectionGroup, where, onSnapshot, and } from 'firebase/firestore'
...
const DairyProductOrdersScreen = ({ navigation }) => {
  ...
  const HandleSearch = () => {
    ...
    const filteredData = originalData.filter((item) => {
      ...
      return item.product.productName.toLowerCase().includes(query)
        ...
        || item.product.owner?.fullname.toLowerCase().includes(query)
        || item.product.owner?.businessName.toLowerCase().includes(query)
        || item.product.owner?.address.toLowerCase().includes(query)
    })
  ...
  const HandleOnPress = (item) => {
    const payload = { order: item, fromOrderHistory: true }
