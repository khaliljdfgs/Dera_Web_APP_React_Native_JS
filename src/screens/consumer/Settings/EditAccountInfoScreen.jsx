  ...
  const HandleSave = async () => {
    ...
    const UpdateUserInformation = async () => {
      ...
      try {
        ...
        setAlert({
          title: 'Success',
          message: email.trim() !== userEmail || password.trim() !== ''
            ? 'Account Information Updated Successfully! Please re-login to see the changes.'
            : 'Account Information Updated Successfully!',
          positiveLabel: 'OK',
          positiveCallback: async () => {
            if (email.trim() !== userEmail || password.trim() !== '') {
              const ENDPOINT = await AsyncStorage.getItem('demo_url') || ''
              await AsyncStorage.clear()
              await AsyncStorage.setItem('demo_url', ENDPOINT)
              const screen = ROUTES.COMMON.LOGIN
    ...
    const validation = [
      ...
      { field: 'Primary Phone', value: primaryPhone, min: 11, max: 11, regExp: REGULAR_EXPRESSION.PHONE },
