...
const SettingsScreen = ({ navigation }) => {
  ...
  const HandleEditAccountInfo = () => navigation.navigate(ROUTES.CONSUMER.EDIT_ACCOUNT_INFO)
  const HandleLocationAndAddress = () => navigation.navigate(ROUTES.CONSUMER.LOCATION_AND_ADDRESS)
  const HandleDairyProduct = () => navigation.navigate(ROUTES.CONSUMER.DAIRY_PRODUCT_ORDERS)
  const HandleLivestockHistory = () => navigation.navigate(ROUTES.CONSUMER.LIVESTOCK_HISTORY)
...

          {
            [
              ...
              },
              {
                ...
                onPress: HandleLivestockHistory
              },
            ].map((item, index, data) => (
              <View style={index > data.length - 2 ? styles.patch : {}} key={index}>
                <Pressable onPress={item.onPress}
