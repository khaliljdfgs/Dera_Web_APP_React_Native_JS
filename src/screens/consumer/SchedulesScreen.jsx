...
import ScheduleListener from '../../listeners/consumer/ScheduleListener'
...
const ScheduleScreen = ({ navigation }) => {
    ...
    if (item.isSubscription) {
      ...
      if (index === -1) {
        ...
        if (item.dailyStatus[index] === DAIRY_PRODUCT_ORDER_SUBSCRIPTION_STATUS.DELIVERED) {
          return [item.dailyStatus[index], index]
        }
        return ['Active', index]
      } else {
        return [item.dailyStatus[index], index]
  ...
  const Setup = async () => {
    ...
    scheduleData.forEach((item) => {
      item.placedBy = currentUser
      item.owner = deraUsers.find((user) => user.id === item.owner)
    ...
    scheduleData.map((item) => {
      if (item.isSingleOrder) {
        if (moment(prepareDate(item.confirmedOn)).isBefore(moment(), 'days') && item.status === DAIRY_PRODUCT_ORDER_STATUS.CONFIRMED) {
    ...
    filteredScheduleData.map((item) => {
      ...
      if (_status) {
        item.statusChip = _status[0]
        item.__index = _status[1]
      }
    })
      ...
      <TouchableOpacity style={styles.itemContainer} activeOpacity={0.65} onPress={() => HandleOnPress(item)}>
        ...
        <View style={{ paddingVertical: 8, paddingHorizontal: 12, gap: 8 }}>
          ...
          <View style={styles.detailsContainer}>
            <View style={styles.detailCellContainer}>
              <Text style={styles.itemAttributeValue} numberOfLines={1} ellipsizeMode='tail'>
                <Text style={{ fontFamily: 'NunitoSans-SemiBold' }}>Qunatity:</Text>
          ...
          <View style={{ ...styles.detailCellContainer, alignItems: 'flex-start' }}>
            <View style={{ ...styles.detailCellContainer }}>
              <MaterialIcons name='category' size={16} color={COLORS.COLOR_01} />
              <Text style={styles.itemAttributeValue} numberOfLines={1} ellipsizeMode='tail'>
                {item.isSubscription ? 'Subscription' : 'Single Order'}
              </Text>
            </View>

            <TouchableOpacity style={{
              backgroundColor: item.statusChip?.toLowerCase() === 'active' || item.statusChip?.toLowerCase() === 'confirmed' ? COLORS.COLOR_01 : `${COLORS.COLOR_01}99`,
              ...
              item.statusChip?.toLowerCase() === 'active' || item.statusChip?.toLowerCase() === 'confirmed' ? 0.62 : 1
            } onPress={() => {
              if (item.statusChip?.toLowerCase() === 'active' || item.statusChip?.toLowerCase() === 'confirmed') {
                navigation.navigate(ROUTES.CONSUMER.QR_CODE, { data: item })
              }
            }}>
              <MaterialIcons name='qr-code' size={18} color={COLORS.COLOR_06} />
              <Text style={{ color: COLORS.COLOR_06, fontFamily: 'NunitoSans-Regular', fontSize: 15 }}>QR Code</Text>
            </TouchableOpacity>
          </View>
        </View>
      </TouchableOpacity>
